...
import {
    ...
    createProjects, updateProject, deleteProject, getDailyWorkJSONAdmin, getDailyWorkJSON
...
function MainPage() {
    ...
    const handleDataFetch = async () => {
        ...
        try {
            const response = await getDailyWorkJSON();
            if (response) {
                // 데이터를 JSON 파일로 다운로드
                downloadUserDataAsJson(response);
            }
        } catch (error) {
            console.error('데이터 가져오기 실패:', error);
        }
    };
    const downloadUserDataAsJson = (userData) => {
    ...
    return (   //TODO: 컴포넌트로 분리해야하ont만 귀찮. 미래의 나야 미안해 뺨 씨게 한대 때리렴
        <>
                ...
                <div className="content">
                    <div className="table-header">
                        ...
                        <div className="button-group2">
                            <button className="add-button" onClick={() => navigate('/daily-work/new')}>
                                <span>추가하기</span>
                            </button>
                            <button onClick={() => handleDataFetch()} className="btn btn-primary btn-sm">
                                JSON 다운로드
                            </button>
                        </div>
                    </div>
                                        ...
                                        {createEmptyRows(dailyData?.[idx]?.monthlyGoals, Math.max(1, dailyData?.[idx]?.monthlyGoals?.length || 0) + 1).map((goal, index) => (
                                            <li key={index} className="monthly-goal-item"
                                                ...
                                                {editingMonthlyGoal?.index === index && editingMonthlyGoal?.date === dateInfo.date ? (
                                                    ...
                                                    >
                              <span
                                  className={!goal.id && (!goal.goals || goal.goals === '-') ? 'empty-cell' : 'filled-cell'}>
                                {goal.goals || '-'}
                              </span>
                                                    </div>
                                                )}
                                        ...
                                        {createEmptyRows(dailyData?.[idx]?.projects, Math.max(4, dailyData?.[idx]?.projects?.length || 0) + 1).map((project, index) => (
                                            <li key={index} className="project-item"
                                                ...
                                                {editingProject?.index === index && editingProject?.date === dateInfo.date ? (
                                                    ...
                                                    >
                              <span
                                  className={!project.id && (!project.goals || project.goals === '-') ? 'empty-cell' : 'filled-cell'}>
                                {project.goals || '-'}
                              </span>
                                                    </div>
                                                )}
                                        ...
                                        {createEmptyRows(dailyData?.[idx]?.routineJobs, Math.max(4, dailyData?.[idx]?.routineJobs?.length || 0) + 1).map((job, index) => (
                                            <li key={index} className="routine-job-item"
                                                ...
                                                {editingRoutineJob?.index === index && editingRoutineJob?.date === dateInfo.date ? (
                                                    ...
                                                    >
                              <span
                                  className={!job.id && (!job.goals || job.goals === '-') ? 'empty-cell' : 'filled-cell'}>
                                {job.goals || '-'}
                              </span>
                                                    </div>
                                                )}
                                        ...
                                        {createEmptyRows(dailyData?.[idx]?.dailyWorks, Math.max(9, dailyData?.[idx]?.dailyWorks?.length || 0) + 2).map((work, index) => (
                                            <li key={index} className="daily-work-item"
                                                ...
                                                {editingCell?.index === index && editingCell?.date === dateInfo.date ? (
                                                    ...
                                                    >
                              <span
                                  className={!work.id && (!work.work || work.work === '-') ? 'empty-cell' : 'filled-cell'}>
                                {work.work || '-'}
                              </span>
                                                    </div>
                                                )}
